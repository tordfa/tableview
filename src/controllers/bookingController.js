export async function tenantExists(tenant_url) {
  const tenantExistsUrl = process.env.REACT_APP_HOST_URL + `/api/tenantexists`
  try {
    const response = await fetch(tenantExistsUrl, {
      credentials: 'include',
      method: "POST",
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ tenant_url: tenant_url})
    });
    if (!response.ok) {
